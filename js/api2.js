                    ...
                    response.json().then(function (data) {
                        ...
                        var homeHTML = `
                            <div class="row">
                                ...
                                <p>Start : ${startDate}</p>
                                <p>End : ${endDate}</p>
                                <a class="waves-effect waves-light btn" href="#schedule">See schedule</a>
                            </div>
                            <div class="row">
                        ...
                        data.seasons.forEach(function (season) {
                            if (season.winner != null) {
                                ...
                                var winner = `<a href="./teams.html?id=${season.winner.id}">${season.winner.name}</a>`;
                                // console.log(season);
                                // url = season.winner.crestUrl.replace(/^http:\/\//i, 'https://');
                            } else {
                                var winner = "-";
                            }
                            var startDate = new Date(season.startDate).toDateString();
                            var endDate = new Date(season.endDate).toDateString();
                            homeHTML += `
                                <tr>
...
function getSchedules() {
    if ("caches" in window) {
        caches.match(base_url + "v2/competitions/2021/matches").then(
            function (response) {
                if (response) {
                    response.json().then(function (data) {
    ...
    fetch(base_url + "v2/competitions/2021/matches?dateTo=" + endDate + "&dateFrom=" + startDate, {
        ...
        .then(function (data) {
            ...
            if (data.matches.length != 0) {
                ...
                console.log(document.getElementById("schedules"));
                document.getElementById("schedules").innerHTML += scheduleHTML;
            }
        }).catch(error);
}
